...
function generateBeatmap(msg: Message, data: any) {
	...
	axios.get(`https://osu.ppy.sh/osu/${data.beatmap.id}`)
		.then((res: any) => {
			...
			colours.getColours(url)
				.then(async (colour) => {
					let colourReadable = colours.toReadable(colour.foreground, colour.background);
					colour.foreground = colourReadable.foreground;
					colour.background = colourReadable.background;
					ctx.fillStyle = colour.background.getHEX();
					...
					ctx.fillStyle = colour.foreground.getHEX();
					...
					// title
					ctx.fillStyle = colour.foreground.getHEX();
					ctx.strokeStyle = colour.foreground.getHEX();
					ctx.lineWidth = 2;
					...
					svgFile = svgFile.replace(/fill="[#\.a-zA-Z0-9]+"/g, `fill="${colour.foreground.getHEX()}"`);
					...
					ctx.beginPath();
					ctx.fillStyle = colour.foreground.getHEX() + '31';
					...
					for (i = 0; i < data.beatmaps.length; i++) {
						...
						if (data.beatmaps[i].difficulty_rating == data.beatmap.difficulty_rating) {
							ctx.globalAlpha = 1;
							ctx.beginPath();
							ctx.fillStyle = colour.foreground.getHEX() + '21';
